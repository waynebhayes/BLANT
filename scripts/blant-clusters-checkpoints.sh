...
USAGE="USAGE: $BASENAME [OPTIONS] blant.exe M Ks network.el
...
[ $# -ne 4 ] && die "expecting exactly 5 mandatory arguments, but you supplied $#"
...
DEBUG=true # set to true to store BLANT output
...
for k in "${Ks[@]}"; do
	hawk ' # This first awk runs quite quickly and does not use much RAM
	    BEGIN{onlyBestOrbit='$ONLY_BEST_ORBIT';
	...
	sort -nr | # > $BLANT_FILES/clus$k.sorted  # sorted cluster-counts of all the nodes, largest-to-smallest
	hawk ' # This second awk can require large amounts of RAM and CPU, eg 10GB and 12-24h for 9wiki-topcats
	    ...
	    function AppendNeighbors(u, origin,    v,oldOrder,Slen,maxEdges,p) {
		Slen=length(S);maxEdges=1
		if (Slen>0) maxEdges=choose(Slen+1,2)
		oldOrder=PROCINFO["sorted_in"];
		PROCINFO["sorted_in"]="randsort";
		if(u in neighbors) {
		    for (v in neighbors[u]) {
			if(!(v in visitedQ) && highRelClusCount(u, v)) {
			    p=100*(EdgesIntoS(v)+edgeCount)/maxEdges
				PQpush("Q",p,v)
			    visitedQ[v]=1;
			}
		    }
		...
		function getNextElementLazy(       u,v,maxEdges,p){
			u=PQpop("Q")
			Slen=length(S);maxEdges=1
			if (Slen>0) maxEdges=choose(Slen+1,2)
			p=100*(EdgesIntoS(u)+edgeCount)/maxEdges
			PQpush("Q",p,u)
			v=PQpop("Q")
			while(u != v){
				u=v
		...
		function checkPointSolution(       v,Slen,maxEdges){
			delete S[u]; edgeCount-=newEdgeHits
		    ...
		    if(Slen>='$minClus') {
				...
				for(v in S) {cluster[numClus][v]=1; printf " %s", v}
				print ""
				if('$ONLY_ONE') exit;
			...
			S[u]=1; edgeCount+=EdgesIntoS(u)
			Slen = length(S);
			maxEdges = choose(Slen,2);
			prev=edgeCount/maxEdges
		}
		...
		numClus=0;
		for(start=1; start<='$ITERATIONS'; start++) { # look for a cluster starting on line "start". 
		    origin=node[start];
		    ...
		    PQdelloc("Q");PQalloc("Q"); #Ensure the queue is empty
			PQpush("Q", 1, origin);
		    visitedQ[origin] = 1;
		    edgeCount = 0;
			prev=1.0;
		    while(PQlength("Q")>0){
				u = getNextElementLazy()
				...
				if (Slen > 1){
					maxEdges = choose(Slen,2);
					if(edgeCount/maxEdges < 10*'$meanED') break;
					if(edgeCount/maxEdges < prev*0.95 || (prev==1.0 && edgeCount/maxEdges < prev)) checkPointSolution()
					AppendNeighbors(u, origin);
				} else AppendNeighbors(u, origin);
		    }
		}
	...
	sort $SUBFINAL_SORT > $TMPDIR/subfinal$k.out & # sort by number of nodes, then by first node in the list
done
for k in "${Ks[@]}"; do
	wait; (( BLANT_EXIT_CODE += $? ))
done
