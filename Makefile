...
test_blant:
	...
	for k in 3 4 5 6 7 8; do export k; echo -n "$$k: "; ./blant -t 4 -og -s 10000000 -k $$k syeast.el | bash -c "paste - <(unxz < syeast.gdv.k$$k.txt.xz)" | hawk '{cols=NF/2;for(i=1;i<=cols;i++)if($$i>1000&&$$(cols+i)>1000)printf "%.9f\n", 1-MIN($$i,$$(cols+i))/MAX($$i,$$(cols+i))}' | stats | sed -e 's/#/num/' -e 's/var.*//' | named-next-col '{if(num<1000 || mean>.005*'$$k' || max>0.2 || stdDev>0.005*'$$k'){printf "BEYOND TOLERANCE:\n%s\n",$$0;exit(1);}else print $$0 }' || break; done
