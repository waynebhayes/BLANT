...
SYNOPSIS (things inside {} are mandatory; those inside [] are optional; if no in graph file name is given, it is read from the standard input):
...
Similar to ODV format above, except for each node, the columns are the number of times that node touches a praticular *graphlet, **independent of which orbit is touched**.* **NOTE**: beware that many authors use the term GDV when they are in fact referring to an ODV. BLANT is more precise in clearly distinguishing between the two.
