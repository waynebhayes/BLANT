...
void canon_map(void){
	...
	// printf("\n\n\ni = %d\n", i);
	if(canonPerm == 0) {
	    G = TinyGraphSelfAlloc(k);
	    Int2TinyGraph(G, i);
	    int nodeArray[k], distArray[k], connected = (TinyGraphBFS(G, 0, k, nodeArray, distArray) == k);

		// printf(" TinyGraphNumEdges(G) %d \n",  TinyGraphNumEdges(G));
		// printf(" Node Array = ");
		// for(int x=0; x<k; x++) {
		// 	printf(" %d ", nodeArray[x]);
		// }
		// printf("\n Dist Array = ");
		// for(int x=0; x<k; x++) {
		// 	printf(" %d ", distArray[x]);
		// }

	    fprintf(fcanon, "\t%c %d", '0'+connected, TinyGraphNumEdges(G));
	    int u,v,sep='\t';
	    for(u=0;u<k;u++)for(v=u+1-G->selfLoops;v<k;v++) {
