// This software is part of github.com/waynebhayes/BLANT, and is Copyright(C)
// Wayne B. Hayes 2025, under the GNU LGPL 3.0 (GNU Lesser General Public
// License, version 3, 2007), a copy of which is contained at the top of the
// repo.
#include "blant.h"
...
#include <time.h>
#include <unistd.h>
Boolean _earlyAbort; // Can be set true by anybody anywhere, and they're
                     // responsible for producing a warning as to why
...
void RandomSeed(long seed) {
  if (mt19937Seed)
    Mt19937Free(mt19937Seed);
  mt19937Seed = Mt19937Alloc(seed);
}
double RandomUniform(void) { return Mt19937NextDouble(mt19937Seed); }
#else
#include "rand48.h"
...
Boolean _useComplement; // to use the complement graph (DEPRECATED, FIND ANOTHER
                        // LETTER)
...
// _degreeVector[g][v], NOT [v][g]. We do this simply because we know the length
...
static unsigned int *
    *_componentList; // list of lists of components, largest to smallest.
static double _totalCombinations, *_combinations, *_probOfComponent;
SET **_componentSet;

// number of parallel threads required, and the maximum allowed at one time (set
// to sysconf(_SC_NPROCESSORS_ONLN) later in main).
...
static int InitializeConnectedComponents(GRAPH *G) {
  ...
  _componentSize =
      Ocalloc(G->n, sizeof(int)); // probably bigger than it needs to be but...
  ...
  for (v = 0; v < G->n; v++)
    if (!SetIn(visited, v)) {
      ...
      GraphVisitCC(G, v, _componentSet[_numConnectedComponents],
                   Varray + nextStart,
                   _componentSize + _numConnectedComponents);
  ...
  for (i = 0; i < _numConnectedComponents; i++) {
    ...
    if (cumulativeProb > 1) {
      assert(cumulativeProb - 1.0 <
             _numConnectedComponents * 1e-15); // allow some roundoff error
      cumulativeProb = _cumulativeProb[i] = 1.0;
    }
    // printf("Component %d has %d nodes and probability %lf, cumulative prob
...
static void InitializeStarMotifs(GRAPH *G) {
  ...
  for (i = 0; i < G->n; i++)
    _totalStarMotifs += CombinChooseDouble(GraphDegree(G, i), _k - 1);
  if (_totalStarMotifs == 0)
    Warning("cannot estimate absolute graphlet count because this network has "
            "no star motifs");
...
const char *SampleMethodStr(void) {
  ...
  case SAMPLE_MCMC:
    if (_sampleSubmethod == SAMPLE_MCMC_EC)
      ...
      return (_MCMC_EVERY_EDGE ? "MCMC:E" : "MCMC");
    break;
...
// _MCMC_L represents the length of the sliding window in d graphlets for
...
void initializeMCMC(GRAPH *G, int k, unsigned long numSamples) {
  ...
  for (i = 0; i < G->numEdges; i++)
    if (_componentSize[_whichComponent[G->edgeList[2 * i]]] >= k)
      validEdgeCount++;
  _samplesPerEdge = (numSamples + (validEdgeCount / 2)) /
                    validEdgeCount; // Division rounding up for samples per edge
  if (_sampleSubmethod == SAMPLE_MCMC_EC) {
...
void finalize(GRAPH *G, unsigned long numSamples) {
  ...
  for (i = 0; i < _numCanon; i++) {
    if (_graphletConcentration[i] < 0.0 || isinf(_graphletConcentration[i]))
      Fatal("_graphletConcentration[%d] %.15g should be neither negative nor "
            "infinite\n",
...
// This function is usually only run at the END after all sampling is finished;
// it converts graphlet frequencies to concentrations or integers based on the
// sampling algorithm and command line arguments. It returns an estimate of the
// total number of connected graphlets in the entire graph
void convertFrequencies(unsigned long numSamples) {
  ...
  case SAMPLE_MCMC:
  case SAMPLE_NODE_EXPANSION:
  case SAMPLE_EDGE_EXPANSION: // case SAMPLE_SEQUENTIAL_CHAINING:
    if (_freqDisplayMode == freq_display_mode_count ||
        _freqDisplayMode == freq_display_mode_estimate_absolute) {
      double totalConcentration = 0;
      for (i = 0; i < _numCanon; i++)
        assert(_graphletConcentration[i] >= 0.0 &&
               !isinf(_graphletConcentration[i]));
      for (i = 0; i < _numCanon; i++)
        totalConcentration += _graphletConcentration[i];
      assert(totalConcentration);
      for (i = 0; i < _numCanon; i++)
        _graphletCount[i] =
            _graphletConcentration[i] / totalConcentration * numSamples;
    }
    break;
  default:
  ...
  if (_sampleMethod == SAMPLE_MCMC || _sampleMethod == SAMPLE_NODE_EXPANSION ||
    ...
    for (i = 0; i < _numCanon; i++)
      if (_canonNumStarMotifs[i] != -1)
        foundStars += _graphletCount[i] * _canonNumStarMotifs[i];
    if (foundStars) {
      assert(_totalStarMotifs);
      _absoluteCountMultiplier = _totalStarMotifs / foundStars;
      total = _absoluteCountMultiplier * numSamples;
      // Note("Absolute Count Multiplier %g; estimated total graphlets is %g",
      // _absoluteCountMultiplier, total);
    } else if (_totalStarMotifs &&
               _quiet < 3) // only print warning if there WERE stars globally
                           // but not locally
      Warning("can't estimate absolute count of graphlets because no stars "
    ...
    for (uint64_t i = start; i < end; i++) {
      ...
      if (_outputMode & graphletDistribution) {
        ...
        if (ProcessGraphlet(G, V, Varray, k, empty_g, weight, accums)) {
          stuck =
              0; // reset stuck counter when finding a newly processed graphlet
        } else {
          // processing failed, ignore sample
          --i;
          if (++stuck > MAX(G->n, _numSamples)) {
            if (_quiet < 2)
              Warning("Sampling aborted for thread %d: no new graphlets "
                      "discovered after %d attempts",
                      threadId, stuck);
            break;
          }
        }
  ...
  for (uint64_t i = 0; i < numSamples; i++) {
    ...
    if (_outputMode & graphletDistribution) {
      ...
      if (ProcessGraphlet(G, V, Varray, k, empty_g, weight, accums)) {
        ...
        if (++stuck > MAX(G->n, _numSamples)) {
          if (_quiet < 2)
            Warning("Sampling aborted: no new graphlets discovered after %lu "
                    "attempts",
                    stuck);
          break;
        }
...
// This is the single-threaded BLANT function. YOU PROBABLY SHOULD NOT CALL
// THIS. Call RunBlantInForks (or RunBlantInThread) instead, it's the top-level
// entry point to call once the graph is finished being input---all the ways of
// reading input call RunBlantIn(Forks|Threads). Note it does stuff even if
...
static int RunBlantFromGraph(int k, unsigned long numSamples, GRAPH *G) {
  ...
  TINY_GRAPH *empty_g = TinyGraphSelfAlloc(k);
#else
  TINY_GRAPH *empty_g = TinyGraphAlloc(
      k, SELF_LOOPS, _directed); // allocate it here once, so functions below
                                 // here don't need to do it repeatedly
#endif
  ...
  // up" into this global copy we initialize below initialize GDV vectors if
  // needed
  if (_outputMode & outputGDV ||
      (_outputMode & communityDetection && _communityMode == 'g')) {
    for (i = 0; i < _numCanon; i++) {
      _graphletDegreeVector[i] = Ocalloc(G->n, sizeof(**_graphletDegreeVector));
      for (j = 0; j < G->n; j++)
        _graphletDegreeVector[i][j] = 0.0;
    }
  }
  // initialize ODV vectors if needed
  if (_outputMode & outputODV ||
      (_outputMode & communityDetection && _communityMode == 'o')) {
    for (i = 0; i < _numOrbits; i++) {
      _orbitDegreeVector[i] = Ocalloc(G->n, sizeof(**_orbitDegreeVector));
      for (j = 0; j < G->n; j++)
        _orbitDegreeVector[i][j] = 0.0;
    }
  ...
  if (_outputMode & graphletDistribution) {
    ...
    for (i = 0; i < _numCanon; i++)
      _graphletDistributionTable[i] = Ocalloc(_numCanon, sizeof(int));
    for (i = 0; i < _numCanon; i++)
      for (j = 0; j < _numCanon; j++)
        _graphletDistributionTable[i][j] = 0;
  }
  ...
  if (_sampleMethod == SAMPLE_INDEX) {
	...
	for(e=0; e<G->numEdges; e++) if(SetIn(needEdge, e)) {
      ...
      // sampling methods in which non-reentrancy has been implemented with the
      // Accumulators struct and thread-local storage (for MCMC)
      _sampleMethod == SAMPLE_EDGE_EXPANSION ||
      _sampleMethod == SAMPLE_NODE_EXPANSION || _sampleMethod == SAMPLE_MCMC) {
    // Apologize if _numThreads > 1 for a sampling method or output mode that
    // isn't yet supported by multithreading
    if (_numThreads > 1) {
      Boolean mcmcThreadSafe = false;

      // MCMC can be parallelized for specific output modes that don't use
      // NodeSetSeenRecently
      if (_sampleMethod == SAMPLE_MCMC) {
        // MCMC_EC uses shared mutable state (_EDGE_COVER_G) and must remain
        // single-threaded
        if (_sampleSubmethod == SAMPLE_MCMC_EC) {
          Note("Multithreading not supported for MCMC_EC (edge cover variant). "
        ...
        // Block MCMC threading for modes that use non-reentrant dedup
        // (NodeSetSeenRecently)
        else if (_outputMode &
                 (indexGraphlets | indexGraphletsRNO | indexOrbits | predict)) {
          Note("Multithreading not supported for MCMC with indexing/predict "
               "modes (non-reentrant dedup). Setting number of threads to 1.");
          _numThreads = 1;
        } else {
          Note("Multithreading (t=%d) not supported for MCMC with the "
               "specified output mode. Setting number of threads to 1.",
               _numThreads);
          _numThreads = 1;
        }
      }
      ...
      if ((_sampleMethod == SAMPLE_EDGE_EXPANSION ||
           _sampleMethod == SAMPLE_NODE_EXPANSION) &&
          !(_outputMode & graphletFrequency || _outputMode & outputGDV ||
            _outputMode & outputODV || _outputMode & indexGraphlets ||
            _outputMode & indexGraphletsRNO || _outputMode & indexOrbits ||
            _outputMode & indexMotifs || _outputMode & indexMotifOrbits ||
            _outputMode & communityDetection)) {
        Note("Multithreading (t=%d) not supported for the specified output "
             "mode with %s. Setting number of threads to 1.",
    ...
    while (!confMet && !_earlyAbort) {
      if (_stopMode == stopOnSamples) {
        ...
        if (_desiredPrec && _quiet < 2) {
          ...
          samplesCounter +=
              batchSize; // Correctly increment samples counter only once.
          batchCounter++;

          // Merge accumulator into globals after each batch (matching
          // multithreaded behavior)
          if (_numThreads == 1) {
            for (i = 0; i < _numCanon; i++) {
              _graphletConcentration[i] +=
                  singleThreadAccums->graphletConcentration[i];
              _graphletCount[i] += singleThreadAccums->graphletCount[i];
              if (_canonNumStarMotifs[i] == -1)
                _canonNumStarMotifs[i] =
                    singleThreadAccums->canonNumStarMotifs[i];
          ...
          if (_batchRawTotalSamples) { // in rare cases a batch may finish with
                                       // no actual samples
            // Even though the samples may not be Normally distributed, the Law
            // of Large Numbers guarantees that for sufficiently large batches,
            ...
            for (int l = 0; l < _numCanon; l++)
              if (SetIn(_connectedCanonicals, l) && _batchRawCount[l]) {
                ...
                if (StatN(sTotal[l]) > 1) {
                  double relInterval =
                      StatConfInterval(sTotal[l], _confidence) /
                      StatMean(sTotal[l]);
            ...
            case mean:
              currentBatchPrecision = _meanPrec;
              break;
            case worst:
              currentBatchPrecision = _worstPrecision;
              break;
            default:
              Fatal("unknown precision mode");
              break;
            }

            if (_quiet < 1) {
              FILE *fp;
              fp = popen("date -Iseconds | sed -e 's/T/ /' -e 's/,/./' -e "
                         "'s/-..:..$//'",
                         "r");
              char buf[BUFSIZ];
            ...
            if (batchCounter >= maxNumBatches ||
                (batchCounter >= minNumBatches &&
                 currentBatchPrecision < _desiredPrec))
              confMet = true; // don't reset the counts if we're done
            else {
              _batchRawTotalSamples = 0;
              for (j = 0; j < _numCanon; j++)
                _batchRawCount[j] = 0;
              // Reset batch counters in single-threaded accumulator for next
              // batch
              if (_numThreads == 1) {
                singleThreadAccums->batchRawTotalSamples = 0;
                for (j = 0; j < _numCanon; j++)
                  singleThreadAccums->batchRawCount[j] = 0;
              }
            }
          } else {
            if (_quiet < 3)
              Warning("invalid batch %d, batchTotal is zero", batchCounter);
          }
        } else {
    ...
    if (_numThreads == 1) {
      for (i = 0; i < _numCanon; i++) {
        ...
        _graphletCount[i] += singleThreadAccums->graphletCount[i];
        if (_canonNumStarMotifs[i] == -1)
          _canonNumStarMotifs[i] = singleThreadAccums->canonNumStarMotifs[i];
      }
      if (_outputMode & outputODV ||
          (_outputMode & communityDetection && _communityMode == 'o')) {
        for (i = 0; i < _numOrbits; i++) {
          ...
          if (singleThreadAccums->orbitDegreeVector[i]) {
            for (j = 0; j < G->n; j++) {
              _orbitDegreeVector[i][j] +=
                  singleThreadAccums->orbitDegreeVector[i][j];
            }
          }
        }
      }
      if (_outputMode & outputGDV ||
        ...
        for (i = 0; i < _numCanon; i++) {
          ...
          if (singleThreadAccums->graphletDegreeVector[i]) {
            for (j = 0; j < G->n; j++) {
              _graphletDegreeVector[i][j] +=
                  singleThreadAccums->graphletDegreeVector[i][j];
            }
          }
        }
      }
        ...
        for (i = 0; i < G->n; i++) {
          if (singleThreadAccums->communityNeighbors[i]) {
            if (!_communityNeighbors[i]) {
              _communityNeighbors[i] =
                  (SET **)Calloc(numCommunities, sizeof(SET *));
            }
            for (j = 0; j < numCommunities; j++) {
              if (singleThreadAccums->communityNeighbors[i][j]) {
                ...
                _communityNeighbors[i][j] = SetUnion(
                    _communityNeighbors[i][j], _communityNeighbors[i][j],
                    singleThreadAccums->communityNeighbors[i][j]);
              }
            }
          }
        }
    ...
    if (!_quiet) {
      ...
      Note("Took %f seconds to sample %lu with %d threads with a sampling "
           "throughput of %.3e iterations/sec in %d batches.",
           elapsed_time, samplesCounter, _numThreads, iterationsPerSecond,
           batchCounter);
    }
	...
	static int batch;
        ...
        {
            if(_window) {
                SampleGraphlet(G, V, Varray, _windowSize, G->n, &_trashAccumulator);
                _numWindowRep = 0;
		...
		static unsigned long stuck;
                double weight = SampleGraphlet(G, V, Varray, k, G->n, &_trashAccumulator);
                if(ProcessGraphlet(G, V, Varray, k, empty_g, weight, &_trashAccumulator)) {
		    stuck = 0;
		    if(_desiredPrec) {
  ...
  if (_window) {
    ...
    if (_windowRep_limit_method)
      HeapFree(_windowRep_limit_heap);
  }
  if ((_sampleMethod == SAMPLE_MCMC || _sampleMethod == SAMPLE_NODE_EXPANSION ||
       _sampleMethod == SAMPLE_EDGE_EXPANSION) &&
      !_window)
  ...
  if ((_outputMode & graphletFrequency || _outputMode & outputGDV ||
       _outputMode & outputODV) &&
      !_window)
  ...
  if (((_outputMode & graphletFrequency &&
        _freqDisplayMode == freq_display_mode_estimate_absolute) ||
       _outputMode & outputGDV || _outputMode & outputODV) &&
      !_window)
    computeAbsoluteMultiplier(numSamples);

  int canon, orbit_index, u, v, c;
  // case indexGraphlets: case indexGraphletsRNO: case indexOrbits: case
  // indexMotifs: case indexMotifOrbits:
  ...
  if (_outputMode & predict) {
    // Turn off ODV and GDV output because we needed them to be COMPUTED but
    // don't want them output in predictMode
    _outputMode &= ~(outputGDV | outputODV);
  }
  // Note: the test for predict mode output MUST be above the test for ODV/GDV
  // output, to turn them off if we're predicting
  ...
  if (_outputMode & outputGDV) {
    for (i = 0; i < G->n; i++) {
      ...
      PrintNode(buf, 0, i);
      for (canon = 0; canon < _numCanon; canon++)
        if (_MCMC_EVERY_EDGE || (_sampleMethod != SAMPLE_MCMC &&
                                 _sampleMethod != SAMPLE_NODE_EXPANSION &&
                                 _sampleMethod != SAMPLE_EDGE_EXPANSION))
      ...
      if (strlen(buf) >= printBuf)
        Fatal("strlen(buf) is %d, compared to printBuf size %d", strlen(buf),
              printBuf);
      puts(buf);
    }
  }
      ...
      for (j = 0; j < _numConnectedOrbits; j++) {
        ...
        if (_MCMC_EVERY_EDGE || (_sampleMethod != SAMPLE_MCMC &&
          ...
          sprintf(
              buf + strlen(buf), " %llu",
              (unsigned long long)llround(_absoluteCountMultiplier *
                                          _orbitDegreeVector[orbit_index][i]));
      }
      if (strlen(buf) >= printBuf)
        Fatal("strlen(buf) is %d, compared to printBuf size %d", strlen(buf),
              printBuf);
    ...
    case 'o':
      for (u = 0; u < G->n; u++) {
        for (c = 0; c < _numConnectedOrbits; c++) {
          ...
          int numPrinted = 0;
          char buf[BUFSIZ];
          if (odv && _communityNeighbors[u] &&
              _communityNeighbors[u][orbit_index] &&
              SetCardinality(_communityNeighbors[u][orbit_index])) {
            int neigh = 0;
            for (j = 0; j < GraphDegree(G, u); j++) {
              char jbuf[BUFSIZ];
              v = GraphNextNeighbor(G, u, &neigh);
              assert(v != -1); // G->neighbor[u][j];
              if (SetIn(_communityNeighbors[u][orbit_index], v)) {
                if (!numPrinted++)
                  sprintf(buf, "%s %d %.15g\t", PrintNode(jbuf, 0, u),
                          orbit_index, odv);
      ...
      for (u = 0; u < G->n; u++) {
        for (c = 0; c < _numCanon; c++)
          if (SetIn(_connectedCanonicals, c)) {
            ...
            int numPrinted = 0;
            char buf[10 * BUFSIZ];
            if (gdv && _communityNeighbors[u] && _communityNeighbors[u][c] &&
                SetCardinality(_communityNeighbors[u][c])) {
              int neigh = 0;
              for (j = 0; j < GraphDegree(G, u); j++) {
                char jbuf[10 * BUFSIZ];
                v = GraphNextNeighbor(G, u, &neigh);
                ...
                if (SetIn(_communityNeighbors[u][c], v)) {
                  if (!numPrinted++)
                    sprintf(buf, "%s %d %lg\t", PrintNode(jbuf, 0, u), c, gdv);
                  PrintNode(buf + strlen(buf), ' ', v);
                }
              }
              assert(-1 == GraphNextNeighbor(G, u, &neigh));
  ...
  // no point in freeing this stuff since we're about to exit; it can take
  // significant time for large graphs.
  if (_outputMode & outputGDV)
    for (i = 0; i < _numCanon; i++)
  ...
  if (_outputMode & outputODV && _MCMC_EVERY_EDGE)
    for (i = 0; i < _numOrbits; i++)
      Free(_orbitDegreeVector[i]);
#endif
...
FILE *ForkBlant(int k, unsigned long numSamples, GRAPH *G) {
  ...
  } else if (pid == 0) // we are the child
  {
    ...
    if (dup(fds[1]) < 0)
      Fatal("dup(2) failed"); // copy the write end of the pipe to fd 1.
    (void)close(fds[1]); // close the original write end of the pipe since it's
                         // been moved to fd 1.

    // For any "counting" mode, use internal numbering when communicating
    // through pipes to the parent
...
static FILE *fpProcs[MAX_POSSIBLE_THREADS]; // these will be the pipes reading
                                            // output of the parallel blants

// This is the primary entry point into BLANT, even if THREADS=1.  We assume
// you've already read the graph into G, and will do whatever is necessary to
// run blant with the number of child processes specified.  Also does some
// sanity checking. [PTHREADS BRANCH IS DEPRECATING THIS]
int RunBlantInForks(int k, unsigned long numSamples, GRAPH *G) {
  int i, j;
  assert(k == _k);
  assert(G->n >= k); // should really ensure at least one connected component
                     // has >=k nodes. TODO
  if (_outputMode & outputGDV ||
      (_outputMode & communityDetection && _communityMode == 'g')) {
    // for(i=0;i<_numCanon;i++) _graphletDegreeVector[i] = Ocalloc(G->n,
    // sizeof(**_graphletDegreeVector));
    for (i = 0; i < _numCanon; i++)
      _graphletDegreeVector[i] = Ocalloc(G->n, sizeof(**_graphletDegreeVector));
  }
  if (_outputMode & outputODV ||
  ...
  if (_sampleMethod == SAMPLE_INDEX || _sampleSubmethod == SAMPLE_MCMC_EC)
    Fatal("The sampling methods INDEX and EDGE_COVER do not yet support "
          "multithreading (feel free to add it!)");
  ...
  if (!_quiet)
    Note("Parent %d starting about %d jobs of about %d samples each", getpid(),
         _numThreads, (int)meanSamplesPerJob);
  ...
  for (i = 0; numSamples > 0 && i < _maxThreads; i++) {
    ...
    if (!_quiet)
      Note("Started job %d of %d samples; %d children running, %ld samples "
           "remaining to take",
           job++, samples, ++procsRunning, numSamples);
  }

  do // this loop reads lines from the parallel child procs, one line read per
     // proc per loop iteration
  {
    char line[1000 * BUFSIZ]; // this is just supposed to be really large
    for (proc = 0; proc < _maxThreads;
         proc++) // process one line from each proc
    {
      if (!fpProcs[proc])
        continue;
      ...
      if (feof(fpProcs[proc])) {
        ...
        --procsRunning;
        if (!_quiet)
          Note("Thead %d finished; jobsDone %d, procsRunning %d", proc,
               jobsDone, procsRunning);
        if (numSamples == 0)
          fpProcs[proc] = NULL; // signify this pointer is finished.
        else {
          unsigned long samples = meanSamplesPerJob;
          if (samples > numSamples)
            samples = numSamples;
          ...
          ++procsRunning;
          if (!_quiet)
            Note("Started job %d (proc %d) of %d samples, %d procs running, "
                 "%ld samples remaining to take",
      ...
      // fprintf(stderr, "Parent received the following line from the child:
      // <%s>\n", line);
      if (_outputMode & graphletFrequency) {
      ...
      if (_outputMode & outputGDV) {
        ...
        while (isdigit(*nextChar))
          nextChar++; // read past current integer
        assert(*nextChar == ' ' ||
               (canon == _numCanon - 1 && *nextChar == '\n'));
        nextChar++;
        for (canon = 0; canon < _numCanon; canon++) {
          assert(isdigit(*nextChar));
      ...
      if (_outputMode & indexGraphlets || _outputMode & indexGraphletsRNO ||
          _outputMode & indexOrbits || _outputMode & indexMotifs ||
      ...
      if (!_outputMode)
        Abort("oops... unknown or unsupported _outputMode in RunBlantInForks "
              "while reading child process");
    }
    lineNum++;
    if (procsRunning > 0)
      Fatal("Need a way to pass StarMotifCounts from children");
...
void BlantAddEdge(int v1, int v2, double weight) {
  ...
  if (_weighted) {
    assert(weight);
    if (!_weights)
      _weights = Malloc(_maxEdges * sizeof(_weights[0]));
  } else
    assert(weight == 0.0);
  ...
  if (_numEdges >= _maxEdges) {
    ...
    if (_weighted)
      _weights = Realloc(_weights, _maxEdges * sizeof(_weights[0]));
  }
  _numNodes = MAX(_numNodes, v1 + 1); // add one since, for example, if we see a
                                      // node numbered 100, numNodes is 101.
  _numNodes = MAX(_numNodes, v2 + 1);
  _pairs[2 * _numEdges] = v1;
  _pairs[2 * _numEdges + 1] = v2;
  if (_weighted)
    _weights[_numEdges] = weight;
  if (_pairs[2 * _numEdges] == _pairs[2 * _numEdges + 1])
    Fatal("BlantAddEdge: edge %d (%d,%d) has equal nodes; cannot have "
          "self-loops\n",
          _numEdges, v1, v2);
  if (_pairs[2 * _numEdges] > _pairs[2 * _numEdges + 1]) {
...
const char *const USAGE_SHORT =
    ...
    "    -s samplingMethod (default MCMC; NBE, EBE!, RES!, AR!, FAYE!, INDEX, "
    "EDGE_COVER)\n"
    "       Note: exclamation mark required after some method names to supress "
    "warnings about them.\n"
    "    -m{outputMode} (default f=frequency; o=ODV, g=GDV, i=index, "
    "cX=community(X=g,o), r=root, p=predict, d=nbrDist\n"
    "    -d{displayModeForCanonicalIDs} (default i=integerOrdinal, o=ORCA, "
    "j=Jesse, b=binary, d=decimal, n=noncanonical)\n"
    "    -r seed (integer)\n"
    "    -F frequency display mode (default a=absolute count (estimated); "
    "c=concentration; n=raw count out of numSamples)\n"
    "    -t N[:M]: (CURRENTLY BROKEN): use threading (parallelism); break the "
    "task up into N jobs (default 1) allowing\n"
    "        at most M to run at one time; M can be anything from 1 to a "
    "compile-time-specified maximum possible value\n"
    "        (MAX_POSSIBLE_THREADS in blant.h), but defaults to 4 to be "
    "conservative.";

const char *const USAGE_LONG =
    "BLANT: Basic Local Alignment of Network Topology (work in progress)\n"
    "PURPOSE: sample graphlets of up to 8 nodes from a graph. Default output "
    "is similar to ORCA, though via stochastic sampling\n"
    "    rather than exaustive enumeration. Our APPROXIMATE results come MUCH "
    "faster than ORCA on large or dense networks.\n"
    "USAGE: blant [OPTIONS] -k numNodes {-[pP] precision | -n numSamples} "
    "graphInputFile\n"
    "where the following are REQUIRED:\n"
    "    numNodes is an integer 3 through 8 inclusive, specifying the size (in "
    "nodes) of graphlets to sample;\n"
    "    precision: desired precision of graphlet frequencies; use p<1 for "
    "fractional precision, p>=1 for #digits (base 10).\n"
    "        Note0: when p>=1, digits can be non-integer, eg 1.5 means "
    "fractional precision 10^(-1.5) or about 3% precision.\n"
    "        Note1: -p limits the MEAN precision across graphlets; -P limits "
    "the worst case precision (not recommended)\n"
    "        Note2: if a 'w' or 'W' is appended after the number specifying "
    "precision, then the mean will be WEIGHTED\n"
    "               by the count--effectively saying you don't care about rare "
    "graphlets and want the frequent ones to have\n"
    "               accurate counts. If an 'l' or 'L' is appended, we weigh by "
    "the logarithm of the count, which is less\n"
    "               Draconian against rare graphlets.\n"
    "        Note3: technically we use confidence intervals. The relative "
    "interval width is set to the precision,\n"
    "               and the default confidence is (1-precision/10). eg. if "
    "p=0.01 (2 digits), confidence is set to 99.9%;\n"
    "               p=0.001 (3 digits) sets confidence to 99.99%. The "
    "confidence is applied to the mean if -p was specified,\n"
    "               or the worst-case if -P was specified (not recommended). "
    "To change the default confidence, use the\n"
    "               -c option (confidence >=1 is assumed to mean percentage\n"
    "    numSamples is the number of graphlet samples to take (large samples "
    "are recommended), except in INDEX sampling mode,\n"
    "	where it specifies the maximum number of samples to take from each "
    "node in the graph.\n"
    "       Note: this option is mutually exclusive to -p or -P.\n"
    "    samplingMethod is:\n"
    "	MCMC (Markov Chain Monte Carlo): Build the first set S of k nodes "
    "using NBE; then randomly remove and add\n"
    "         nodes to S using an MCMC graph walking algorithm with restarts; "
    "gives asymptotically correct relative frequencies\n"
    "         when using purely counting modes like -m{o|g|f}, but biased "
    "counts in indexing modes like -m{i|j} since we remove\n"
    "         duplicates in indexing modes.)\n"
    "	NBE (Node-Based Expansion): pick an edge at random and add it to the "
    "node set S; add new nodes by choosing\n"
    "         uniformly at random from all nodes one step outside S. (slow, "
    "but correct)\n"
    "	EBE (Edge-Based Expansion): pick an edge at random and add its two "
    "nodes to S; add nodes to S by picking an edge\n"
    "         uniformly at random from those emanating from S. (faster than "
    "NBE on dense networks, but more biased)\n"
    "	RES (Lu Bressan's REServoir sampling): also asymptotically correct but "
    "much slower than MCMC.\n"
    "	AR (Accept-Reject): EXTREMELY SLOW but asymptotically correct: pick k "
    "nodes entirely at random, reject if\n"
    "	  resulting graphlet is disconnected (vast majority of such grpahlets "
    "are disconnected, thus VERY SLOW)\n"
    "	INDEX: deterministic: for each node v in the graph, build a "
    "topologically deterministic set of k-graphlets to\n"
    "         be used as indices for seed-and-extend local alignments (using, "
    "eg., our onw Dijkstra-inspired local aligner--\n"
    "         see Dijkstra diretory). When using INDEX sampling, the -n "
    "command-line option specifies the maximum number\n"
    "         of index entries per starting node v.\n"
    "	EDGE_COVER: starting with E=all edges in the input graph, pick one "
    "edge in E and build a k-graphlet using EBE;\n"
    "         then subtract ALL its edges from E. Continue until E is empty. "
    "The goal is to output a short list of graphlets\n"
    "         that, in comglomerate, cover each edge in E at least once.\n"
    "    graphInputFile: graph must be in one of the following formats with "
    "its extension name:\n"
    "	Edgelist (.el), LEDA(.leda), GML (.gml), GraphML (.xml), LGF(.lgf), "
    "CSV(.csv)\n"
    "	(extensions .gz and .xz are automatically decompressed using gunzip "
    "and unxz, respectively)\n"
    ...
    "	f = [default] graphlet {f}requency, estimates the total count of each "
    "type of graphlet.\n"
    "	    sub-option -mf{freqDispMode} can be e{default=estimate), n(integer "
    "count) or c(concentration)\n"
    "	o = ODV (Orbit Degree Vector), identical to ORCA (commonly though "
    "incorrectly called a GDV)\n"
    "	g = GDV (Graphlet Degree Vector) Note this is NOT what is commonly "
    "called a GDV, which is actually an ODV (above).\n"
    "	NOTE: the difference is that an ODV counts the number of nodes that "
    "touch all possible *orbits*, while a GDV lists\n"
    "		only the smaller vector of how many nodes touch each possible "
    "*graphlet* (independent of orbit).\n"
    "	i = {i}ndex: each line is a graphlet with columns: canonical ID, then "
    "k nodes in canonical order; useful since\n"
    "	    two lines with the same first column constitutes a PERFECT k-node "
    "local alignment between the two graphlets.\n"
    "	cX = Community detection where X is g for graphlet (the default) or o "
    "for orbit (which uses FAR more memory!)\n"
    "	    Each line consists of:   node (orbit|graphlet) count [TAB] "
    "neighbors.\n"
    "	r = index with {r}oot node orbit: each line is a canonical ID + the "
    "orbit of the root node, then k nodes in canonical order; produces better "
    "seeds when the index is queried by the alignment algorithm\n"
    "	pO:P = edge Prediction, where O and P are an orbit pair (eg 11:11 when "
    "k=4 for the L3 path from Kovacs et. al.)\n"
    "	d = graphlet neighbor {D}istribution\n"
    "    -d{displayMode: single character controls how canonical IDs are "
    "displayed. (DEFAULT=-mi): \n"
    "	o = ORCA numbering\n"
    "	j = JESSE numbering\n"
    "	b = explicit binary representation of the half-adjacency matrix of the "
    "canonical graphlet\n"
    ...
    "    -t N[:M]: use threading (parallelism); break the task up into N jobs "
    "(default 1) allowing at most M to run at one time.\n"
    "       M can be anything from 1 to a compile-time-specified maximum "
    "possible value (MAX_POSSIBLE_THREADS in blant.h),\n"
    "       but defaults to 4 to be conservative.\n"
    "    -r seed: pick your own random seed\n"
    "    -S is for SANITY TESTING ONLY! It turns off the de-biasing alpha "
    "multipliers and produces BIASED samples\n"
    "    -i FILENAME: file containing \"nodes of interest\"; every graphlet "
    "sampled will have at least one of these nodes\n"
    "    -W windowSize: DEPRECATED. (use '-h' option for more)\n"
    "	-p windowRepSamplingMethod: (DEPRECATED) one of the below\n"
    "	    MIN (Minimizer); MAX (Maximizer); DMIN (Minimizer With Distance); "
    "DMAX (Maximizer with Distance);\n"
    "	    LFMIN (Least Frequent Minimizer); LFMAX (Least Frequent "
    "Maximizer)\n"
    "	-P windowRepIterationMethods is one of: COMB (Combination) or DFS\n"
    "	-l windowRepLimitMethod is one of: [suffix N: limit to Top N satisfied "
    "graphlets]\n"
    "	    DEG (graphlet Total Degree); EDGE (1-step away numEdges)\n"
    "\nOPTIONS specific to -sINDEX sampling mode:\n"
    "   -M max multiplicity (INDEX sampling only: max allowed ambiguous "
    "permutations in indexed graphlets; M=0 means no max)\n"
    ...
    "   -a = sets whether ties are broken alphabetically or reverse "
    "alphabetically";
...
// The main program, which handles multiple threads if requested.  We simply
// fire off a bunch of parallel blant *processes* (not threads, but full
// processes), and simply merge all their outputs together here in the parent.
int main(int argc, char *argv[]) {
  // printf("char %d short %d int %d long %d long long %d Gint %d\n",
  // sizeof(char), sizeof(short), sizeof(int), sizeof(long), sizeof(long long),
  // sizeof(Gint_type));
  //  ENABLE_MEM_DEBUG(); // requires including "mem-debug.h" in blant.h (NOT at
  //  the top of blant.c!)
  ...
  while ((opt = getopt(
    ...
    case 'i':
      ...
      if (!interestFile)
        Fatal("cannot open nodes-of-interest file '%s' while processing -i "
              "option",
              optarg);
      // file will be read later, after we read the graph input file so we have
      // the names
      break;
    case 'F':
      ...
      case 'n':
        ...
        break;
      case 'c':
        _freqDisplayMode = freq_display_mode_concentration;
        break;
      case 'a':
      case 'e':
        _freqDisplayMode = freq_display_mode_estimate_absolute;
      ...
      break;
    case 'm':
      if (_outputMode != undef)
        Fatal("tried to define output mode twice");
      switch (*optarg) {
      ...
      case 'i':
        ...
        break;
      case 'r':
        _outputMode |= indexGraphletsRNO;
        break;
      case 'j':
        _outputMode |= indexOrbits;
        break;
      case 'f':
        _outputMode |= graphletFrequency;
        break;
      case 'g':
        _outputMode |= outputGDV;
        break;
      case 'o':
        _outputMode |= outputODV;
        break;
      ...
      case 'q':
        ...
        break;
      default:
        Fatal("-m%c: unknown output mode \"%c\"", *optarg, *optarg);
        break;
      }
      break;
    case 'd':
      if (_displayMode != undefined)
        Fatal("tried to define canonical display mode twice");
      switch (*optarg) {
      case 'b':
        _displayMode = binary;
        break;
      case 'd':
        _displayMode = decimal;
        break;
      case 'i':
        _displayMode = ordinal;
        break;
      case 'j':
        _displayMode = jesse;
        break;
      case 'o':
        ...
        break;
      case 'n':
        _displayMode = noncanonical;
        break;
      default:
        Fatal("-d%c: unknown canonical display mode:n"
              "\tmodes are i=integer ordinal, d=decimal, b=binary, o=orca, "
              "j=jesse",
              *optarg);
        break;
      }
    ...
    case 't':
      ...
      if (_numThreads > _maxThreads)
        Warning("More threads specified than available on system.");
      break;
    case 'r':
      _seed = atoi(optarg);
      if (_seed == -1)
        Apology("seed -1 ('-r -1' is reserved to mean 'uninitialized'");
      break;
    case 'R':
      _rawCounts = true;
      break;
    case 's':
      if (_sampleMethod != -1)
        Fatal("Tried to define sampling method twice");
      else if (strncmp(optarg, "NBE", 3) == 0)
        _sampleMethod = SAMPLE_NODE_EXPANSION;
      else if (strncmp(optarg, "SEC", 3) ==
               0) //_sampleMethod = SAMPLE_SEQUENTIAL_CHAINING;
        Apology("SEC is no longer supported due to fundamentally unfixable "
                "problems with the method");
      else if (strncmp(optarg, "FAYE", 4) == 0) {
        if (strncmp(optarg, "FAYE!", 5) != 0)
          Warning("FAYE is an ancient variant of NBE and produces counts with "
                  "potentially large biases; suppress this warning by "
                  "appending an exclamation mark");
        _sampleMethod = SAMPLE_FAYE;
      } else if (strncmp(optarg, "EBE", 3) == 0) {
        // if (strncmp(optarg, "EBE!",4) != 0) Warning("EBE is very fast on
        // dense networks but produces counts with potentially extreme biases;
        // suppress this warning by appending an exclamation mark");
        _sampleMethod = SAMPLE_EDGE_EXPANSION;
      } else if (strncmp(optarg, "MCMC", 4) == 0) {
        // if (strncmp(optarg, "MCMC!",5) != 0) Warning("MCMC produces unbiased
        // but high variance graphlet counts; suppress this warning by appending
        // an exclamation mark");
        ...
        if (strncmp(optarg, "RES!", 4) != 0)
          Warning("Reservoir sampling (RES) is unbiased but VERY slow; append "
                  "an exclamation mark to supress this warning");
        _sampleMethod = SAMPLE_RESERVOIR;
      } else if (strncmp(optarg, "AR", 2) == 0) {
        _sampleMethod = SAMPLE_ACCEPT_REJECT;
        if (strncmp(optarg, "AR!", 3) != 0)
          Warning("Accept/Reject sampling (AR) is unbiased but EXTREMELY slow; "
                  "append an exclamation mark to supress this warning");
        ...
        if (strcmp(optarg, "STDIN") == 0)
          ...
          _sampleFile = fopen(_sampleFileName, "r");
        if (!_sampleFile)
          Fatal("Unrecognized sampling method '%s'; recognized options are "
                "NBE, MCMC, EBE, RES, FAYE, AR, or a filename (that can be "
                "'STDIN'), but file '%s' cannot be opened",
                optarg, _sampleFileName);
        _sampleMethod = SAMPLE_FROM_FILE;
      }
      break;
    case 'P':
      _precisionMode = worst; // fall through, do not break
    case 'p':
      if (atof(optarg) < 1) { // user has asked for relative precision
        _desiredPrec = atof(optarg);
        if (_desiredPrec <= 0)
          Fatal("invalid requested precision %g must be in (0,1)",
                _desiredPrec);
        _desiredDigits = -log(_desiredPrec) / log(10);
      } else { // user has requested digits of precision
        _desiredDigits = atof(optarg);
        if (_desiredDigits <= 0)
          Fatal("invalid requested digits of precision %g must be > 0",
                _desiredDigits);
        _desiredPrec = pow(10, -_desiredDigits);
      }
      if (_desiredDigits > 3)
        Warning("requesting more than 3 digits of precision may be infeasible; "
                "you've requested %g",
                _desiredDigits);
      if (_confidence)
        Fatal("Please specify confidence (-c option) AFTER specifying "
              "precision with -p or -P");
    ...
    case 'l':
      ...
      if (strncmp(optarg, "n", 1) == 0 || strncmp(optarg, "N", 1) == 0) {
        _windowRep_limit_neglect_trivial = true;
        optarg += 1;
      }
      if (strncmp(optarg, "DEG", 3) == 0) {
        _windowRep_limit_method = WINDOW_LIMIT_DEGREE;
        optarg += 3;
      } else if (strncmp(optarg, "EDGE", 4) == 0) {
        _windowRep_limit_method = WINDOW_LIMIT_EDGES;
        optarg += 4;
      } else
        Fatal("Unrecognized window limiting method specified. Options are: "
              "-l{DEG}{EDGE}{limit_num}\n");
      _numWindowRepLimit = atoi(optarg);
      if (!_numWindowRepLimit) {
        _numWindowRepLimit = 10;
    ...
    case 'n':
      ...
      if (nSampArg < 0)
        Fatal("%s\nFatal Error: numSamples [%s] must be a non-negative integer",
              USAGE_SHORT, optarg);
      ...
      if (!isdigit(lastChar))
        ...
        case 'g':
        case 'G':
          numSamples *= 1024; // do NOT break, fall through
        case 'm':
        case 'M':
          numSamples *= 1024; // do NOT break, fall through
        case 'k':
        case 'K':
          ...
          break;
        default:
          Fatal("%s\nERROR: numSamples can be appended by k, m, b, or g but "
                "not %c\n%s",
                USAGE_SHORT, lastChar);
    ...
    case 'g':
      if (!GEN_SYN_GRAPH)
        Fatal("Turn on Global Variable GEN_SYN_GRAPH");
      _GRAPH_GEN = true;
      if (_genGraphMethod != -1)
        Fatal("Tried to define synthetic graph generating method twice");
      else if (strncmp(optarg, "NBE", 3) == 0)
        _genGraphMethod = GEN_NODE_EXPANSION;
      else if (strncmp(optarg, "MCMC", 4) == 0)
        Apology(
            "MCMC for Graph Syn is not ready"); // _genGraphMethod = GEN_MCMC;
      else
        Fatal("Unrecognized synthetic graph generating method specified. "
              "Options are: -g{NBE|MCMC}\n");
      break;
    case 'M':
  ...
  if (_orbitNumber != -1) {
    if (_odvFile != NULL) {
      ...
      Fatal("an ODV orbit number was provided, but no ODV file path was "
            "supplied");
    }
  }
  ...
  if (_outputMode == undef)
    _outputMode = graphletFrequency; // default to frequency, which is simplest
  if (_freqDisplayMode == freq_display_mode_undef)
    _freqDisplayMode =
        freq_display_mode_estimate_absolute; // Default to estimating count

  if (numSamples && _desiredPrec)
    Fatal("cannot specify both -n (sample size) and -[Pp] (desired precision)");
  if (!numSamples &&
      !_desiredPrec) { // default to 2 digits of precision at 99.9% confidence
  ...
  if (!_quiet && !(_outputMode & predict_merge))
    Note("Sampling method is %s", SampleMethodStr());

  if (_desiredPrec && _confidence == 0)
    _confidence = (1 - _desiredPrec / 10);
  // if(_desiredPrec && _sampleMethod == SAMPLE_MCMC) Warning("you've chosen
  // MCMC sampling with confidence intervals; EBE is recommended since adjacent
  // MCMC samples are not independent"); // actually it seems to work fine
  ...
  if (_window && _windowSize >= 3) {
    ...
    _numWindowRepArrSize = _MAXnumWindowRep > 0
                               ? MIN(_numWindowRepArrSize, _MAXnumWindowRep)
                               : _numWindowRepArrSize;
    // _windowReps needs true Calloc/Free since they may be realloc'd
    // on-the-fly.
    _windowReps = Calloc(_numWindowRepArrSize, sizeof(int *));
    for (i = 0; i < _numWindowRepArrSize; i++)
      _windowReps[i] = Calloc(_k + 1, sizeof(int));
    if (windowRep_edge_density < 0)
      windowRep_edge_density = 0;
    if (windowRep_edge_density > 1)
      windowRep_edge_density = 1;
    _windowRep_min_num_edge =
        (int)CombinChooseDouble(_k, 2) * windowRep_edge_density;
    if (_windowRep_min_num_edge < 0)
  ...
  if (_sampleMethod == SAMPLE_INDEX || _window) {
    ...
    for (i = 0; i < _numCanon; i++) {
      if (SetIn(_connectedCanonicals, i)) {
        ...
        // the formula is for every unique orbit, multiply the number of
        // permutations by the factorial of how many appearances that unique
        // orbit has if there is one orbit with three nodes and a second orbit
        // with 2 nodes, the number of permutations would be (3!)(2!) NOTE: this
        // is not the most efficient algorithm since it doesn't use hash tables.
        // I didn't want to overcomplicate it because it only happens once per
        // run. however, if speed is important (this currently takes about 5
        // seconds on k=8) this can be sped up
        for (j = 0; j < _k; j++)
          SetAdd(orbit_temp, _orbitList[i][j]);
        ...
        // I know it's inefficient to put multiplicity here instead of around
        // the whole orbit perm calculation code but it increases readability,
        // at least until orbit perm calculation is put into a function
        if (multiplicity == 0 ||
            total_orbit_perms <= multiplicity) { // multiplicity = 0 means any
  ...
  if (!argv[optind]) {
    fpGraph = stdin;
    if (isatty(0) && !_quiet)
      Warning("reading graph input file from terminal, press ^D to finish");
  } else {
    char *graphFileName = argv[optind];
    // readFile doesn't actually READ anything, it only prepares the file for
    // reading by, for example, uncompressing it if it's name ends in ".gz"
  ...
  // The following line points G to the zeroth element of inputG, and from that
  // point onward G can be used as it has always been used; but whenever
  // _directed is true and we want to access the directed version, use (G+1).
  GRAPH *G = &inputG[0];
  ...
  // Always allocate this set; if there are no "nodes of interest" then every
  // node is a possible start done
  _startNodes = Calloc(G->n, sizeof(unsigned));
  _startNodeSet = SetAlloc(G->n);
  if (interestFile) {
    if (_sampleMethod != SAMPLE_NODE_EXPANSION) {
      Warning("sampleMethod is being set to NBE to accomodate "
              "nodes-of-interest file");
      _sampleMethod = SAMPLE_NODE_EXPANSION;
    }
    ...
    while (1 == fscanf(interestFile, "%s", nodeName)) {
      ...
      if (_supportNodeNames) {
        if (!TreeLookup(G->nameDict, (foint)nodeName, &nodeNum))
          Fatal("nodes-of-interest file contains non-existent node '%s'",
                nodeName);
      } else {
        nodeNum.i = atoi(nodeName);
        if (nodeNum.i < 0 || nodeNum.i >= G->n)
          Fatal("nodes-of-interest file contains non-existent node '%d'",
                nodeNum.i);
      }
      if (SetIn(_startNodeSet, nodeNum.i))
        Fatal("nodes-of-interest cannot contain duplicate nodes but we've "
              "already seen '%s'",
              nodeName);
      ...
      if (_numStartNodes > G->n)
        Fatal("nodes-of-interest file contains '%d' entries, which is more "
              "nodes (%d) than input graph",
              _numStartNodes, G->n);
    }
  ...
  if (_outputMode & communityDetection) {
    if (_communityMode == 'o' ||
      ...
      _communityNeighbors = (SET ***)Calloc(
          G->n, sizeof(SET **)); // elements are only allocated when needed
    else
      Fatal("unknown _communityMode %c", _communityMode);
  }
  ...
  if (_windowSampleMethod == WINDOW_SAMPLE_DEG_MAX) {
    ...
    if ((optind + 1) == argc) {
      ...
      if (fp == NULL)
        Fatal("cannot open graph Node Importance File.");
  ...
  if (_GRAPH_GEN) {
    if (_k_small == 0)
      _k_small = _k;
    if (numSamples == 0)
      Fatal("Haven't specified sample size (-n sampled_size)");
    if (_confidence == 0)
      _confidence = 0.05;
    if (_KS_NUMSAMPLES == 0)
      _KS_NUMSAMPLES = 1000;
    if (_GRAPH_GEN_EDGES == 0)
      _GRAPH_GEN_EDGES = G->numEdges;
    if ((optind + 1) == argc) {
      fpSynGraph = fopen(argv[optind++], "w");
      if (fpSynGraph == NULL)
        Fatal("cannot open synthetic graph outputfile.");
    }
