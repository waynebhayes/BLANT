/*
*    Bogacki-Shampine Algorithm RK23
*
...
main()
{
   double A,B,TOL,ALPHA,HMAX,HMIN,H,T,W,K1,K2,K3,K4,R,DELTA;
   int I,N,OK;
   FILE *OUP[1];
   ...
   if (OK) {
      ...
      while ((T < B) && OK) {
         ...
         K1 = H*F(T,W);
         K2 = H*F(T+H/2,W+K1/2);
         K3 = H*F(T+3*H/4,W+3*K2/4);
         K4 = H*F(T+H,W+(2*K1+3*K2+4*K3)/9);

         /* STEP 4 */
         R = absval(5*K1/72-K2/12-K3/9+K4/8)/H;
         /* STEP 5 */
         if (R <= TOL) {
            /* STEP 6 */
            /* APPROXIMATION ACCEPTED */
            T = T + H;
            W = W+2*K1/9+K2/3+4*K3/9;
            /* STEP 7 */
            fprintf(*OUP, "%12.7f %11.7f %11.7f %11.7f\n", T, W, H, R);
         }
         /* STEP 8 */
         /* TO AVOID UNDERFLOW */
         if (R > 1.0E-20) DELTA = 0.84 * exp(0.5 * log(TOL / R));
         else DELTA = 10.0;
         //printf( "%f\n Delta=", DELTA);
         /* STEP 9 */
         /* CALCULATE NEW H */
...
void INPUT(int *OK, double *A, double *B, double *ALPHA, double *TOL, double *HMIN, double *HMAX, int *N)
{
   ...
   printf("This is the Bogacki-Shampine Method.\n");
   *OK = false;
...
void OUTPUT(FILE **OUP)
{
   ...
   fprintf(*OUP, "BOGACKI-SHAMPINE METHOD\n\n");
