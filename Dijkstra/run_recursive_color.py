import argparse
import recalignment_color as recalignment
import seeding
import builder
import os.path
...
def initParser():
    ...
    parser.add_argument("-sb", "--simbound", required=False, default = "0.0", help ="lower bound for similarity")
    ...
    parser.add_argument('-cf1', "--colorfile1",required=False, default = None, help="Colors for graph1")
    parser.add_argument('-cf2', "--colorfile2",required=False, default = None, help="Colors for graph2")
...
if __name__ == '__main__':
    ...
    if args.colorfile1:
        builder.read_color(graph1, args.colorfile1)
    else:
        graph1.colors = {} 
    
    if args.colorfile2:
        builder.read_color(graph2, args.colorfile2)
    else:
        graph2.colors = {} 
    ...
    for seed in seeding.generate_seed(g1_seed_file,g2_seed_file):
        ...
        recalignment.rec_align(graph1, graph2, seeding.get_aligned_seed(zip(*seed),graph1, graph2), sims, ec_mode, ed, e1, sb, delta, alpha, seednum, outputdir="", alignstop=-1, timestop=timestop_arg, debug=args.debugval)    
